...
package object json {
  ...
  implicit lazy val taskJsonFormat: JsonFormat[Task] = new JsonFormat[Task] {
    ...
    private implicit lazy val deregisterAmiAndDeleteSnapshotsTaskFormat: JsonFormat[DeregisterAmiAndDeleteSnapshotsTask] = jsonFormat1(DeregisterAmiAndDeleteSnapshotsTask)
    ...
    override def read(json: JsValue): Task = {
      json.asJsObject.fields("task").convertTo[String] match {
        ...
        case "DeregisterAmiAndDeleteSnapshots" => deregisterAmiAndDeleteSnapshotsTaskFormat.read(json)
    ...
    override def write(obj: Task): JsValue = obj match {
      ...
      case task: DeregisterAmiAndDeleteSnapshotsTask => writeTask("DeregisterAmiAndDeleteSnapshots", task)
